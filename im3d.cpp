...
bool Im3d::GizmoSelectionRectangle(Id _id, float _selection_[4 * 3])
{
	...
	// edge
	{
		float selection_edge_cpy[4 * 3];
		memcpy(selection_edge_cpy, selection_edge, sizeof(selection_edge_cpy));
		drawNBehavior(handlesEdge, handlesEdgeCount);

		for (size_t i = 0; i < handlesCornerCount; ++i)
		{
			int const j0 = i * 3;
			int const j1 = ((i + 1) % handlesCornerCount) * 3;
			for (size_t k = 0; k < 3; ++k)
			{
				auto const d = selection_edge[j0 + k] - selection_edge_cpy[j0 + k];
				_selection_[j0 + k] += d;
				_selection_[j1 + k] += d;
			}
		}
